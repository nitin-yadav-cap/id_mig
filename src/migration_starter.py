...
lines = read_conf('/Users/nitin/Documents/cap/id_mig/src/test_conf.txt')
for line in lines:
    sp = line.split(':')
    ...
    for s in columns:
        s
    #print(line)
    # commented out since source table target mapping logical id_str was being reset
    if table_name == "source_target_table_mapping":
        continue
...
custom_logic.handle_stdcm_id_str()
custom_logic.handle_summary_table_column_definition()
custom_logic.handle_union_mapping_id_str()
custom_logic.handle_fact_kpis_id_str()
custom_logic.handle_source_target_table_mapping()
